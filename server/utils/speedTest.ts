 ...
 * 测试单个节点
 */
export async function testNode(node: any): Promise<SpeedTestResult> {
    try {
        const { reachable, latency } = await testTCPConnection(
            node.server,
            node.port,
            5000
        )
 ...
 * 获取可达节点（按延迟排序）
 */
export function getReachableNodes(results: SpeedTestResult[]): SpeedTestResult[] {
    return results
 ...
 * 获取统计信息
 */
export function getSpeedTestStats(results: SpeedTestResult[]) {
