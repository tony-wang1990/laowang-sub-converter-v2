 ...
 * 使用远程API进行转换（带故障转移）
 */
export async function remoteConvert(params: ConversionParams): Promise<string> {
 ...
 * 构建远程后端URL
 */
function buildBackendUrl(baseUrl: string, params: ConversionParams): string {
 ...
 * 解析多个订阅URL
 */
export function parseMultipleUrls(urlParam: string): string[] {
    // 支持换行符或 | 分隔
    return urlParam
        .split(/[\n|]/)
        .map(u => u.trim())
...
export async function smartConvert(
    localConvertFn: (params: ConversionParams) => Promise<string>,
