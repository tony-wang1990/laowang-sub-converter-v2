...
function parseHysteria(uri: string): ProxyNode | null {
    try {
        // 检测协议版本
...
// WireGuard 解析
function parseWireGuard(uri: string): ProxyNode | null {
    try {
        const url = new URL(uri)
        const params = url.searchParams
...
// Brook 解析
function parseBrook(uri: string): ProxyNode | null {
    try {
...
// Snell 解析 V4
function parseSnell(uri: string): ProxyNode | null {
    try {
        const url = new URL(uri)
        const params = url.searchParams
