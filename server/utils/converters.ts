...
// 转换核心逻辑
export function convertToTarget(nodes: ProxyNode[], target: string, options: ConverterOptions): string {
    switch (target) {
        case 'clash':
        case 'clashmeta':
        case 'clashverge': // New
...
function convertToClash(nodes: ProxyNode[], options: ConverterOptions): string {
    const proxies = nodes.map(node => {
        ...
        switch (node.type) {
            case 'ss':
            case 'ss2022':
                const ss: any = { ...baseProxy, type: 'ss', cipher: node.method, password: node.password }
                if (node.plugin) {
                    if (node.plugin.includes(';')) {
                        ...
                        if (node.pluginOpts) ss['plugin-opts'] = parsePluginOpts(node.pluginOpts)
                    }
                }
                return ss
            ...
            case 'trojan':
                ...
                if (node.alpn) trojan.alpn = node.alpn
        ...
        'proxy-groups': [
            {
                name: '🚀 节点选择',
                type: 'select',
                proxies: ['♻️ 自动选择', 'DIRECT', ...proxies.map((p: any) => p.name)]
            },
            {
                name: '♻️ 自动选择',
                type: 'url-test',
...
// Surge 格式 - 改进版
function convertToSurge(nodes: ProxyNode[], options: ConverterOptions): string {
    const proxies = nodes.map(node => {
        switch (node.type) {
            case 'ss':
                let ss = `${node.name} = ss, ${node.server}, ${node.port}, encrypt-method=${node.method}, password=${node.password}`
                if (node.plugin) ss += `, plugin=${node.plugin}`
                if (options.udp) ss += ', udp-relay=true'
                return ss
            ...
            case 'vmess':
                ...
                if (node.tls) {
                    vmess += ', tls=true'
                    if (node.sni) vmess += `, sni=${node.sni}`
                }
                if (node.network === 'ws' && node.ws) {
                    vmess += ', ws=true'
                    if (node.ws.path) vmess += `, ws-path=${node.ws.path}`
                    if (node.ws.headers?.Host) vmess += `, ws-headers=Host:${node.ws.headers.Host}`
                }
...
// Quantumult X 格式 - 优化版
function convertToQuantumultX(nodes: ProxyNode[], options: ConverterOptions): string {
    const proxies = nodes.map(node => {
        switch (node.type) {
            case 'ss':
            ...
            case 'vmess':
                ...
                if (node.tls) vmess += ', obfs=over-tls'
                if (node.network === 'ws' && node.ws) {
                    vmess += ', obfs=ws'
                    if (node.ws.path) vmess += `, obfs-uri=${node.ws.path}`
                    if (node.ws.headers?.Host) vmess += `, obfs-host=${node.ws.headers.Host}`
                }
            ...
            case 'vless':
                ...
                if (node.tls) vless += ', obfs=over-tls'
...
// Loon 格式 - 改进版
function convertToLoon(nodes: ProxyNode[], options: ConverterOptions): string {
    const proxies = nodes.map(node => {
        switch (node.type) {
            case 'ss':
            ...
            case 'vmess':
                ...
                if (node.network === 'ws' && node.ws) {
                    ...
                    if (node.ws.headers?.Host) vmess += `,host=${node.ws.headers.Host}`
                }
    ...
    const links = nodes.map(node => {
        switch (node.type) {
            ...
            case 'ssr':
                ...
                if (node.protocolParam) ssrParams.set('protoparam', Buffer.from(node.protocolParam!).toString('base64'))
                if (node.obfsParam) ssrParams.set('obfsparam', Buffer.from(node.obfsParam!).toString('base64'))
            ...
            case 'vmess':
                ...
                if (node.ws) {
                    ...
                    if (node.ws.headers?.Host) vmessData.host = node.ws.headers.Host
                } else if (node.h2) {
                    vmessData.path = node.h2.path
                    if (node.h2.host?.length) vmessData.host = node.h2.host.join(',')
                } else if (node.grpc) {
            ...
            case 'vless':
                ...
                if (node.reality) {
                    ...
                    vless += '&security=tls'
                    if (node.sni) vless += `&sni=${encodeURIComponent(node.sni)}`
                    if (node.alpn) vless += `&alpn=${encodeURIComponent(node.alpn.join(','))}`
                    if (node.fingerprint) vless += `&fp=${node.fingerprint}`
                }
                if (node.flow) vless += `&flow=${node.flow}`
                if (node.ws) {
                    if (node.ws.path) vless += `&path=${encodeURIComponent(node.ws.path)}`
                    if (node.ws.headers?.Host) vless += `&host=${encodeURIComponent(node.ws.headers.Host)}`
                } else if (node.grpc) {
                    if (node.grpc.serviceName) vless += `&serviceName=${encodeURIComponent(node.grpc.serviceName)}`
                } else if (node.h2) {
                    if (node.h2.path) vless += `&path=${encodeURIComponent(node.h2.path)}`
                    if (node.h2.host?.length) vless += `&host=${encodeURIComponent(node.h2.host.join(','))}`
                }
            ...
            case 'trojan':
                ...
                if (node.alpn) trojanParams.set('alpn', node.alpn.join(','))
                if (node.fingerprint) trojanParams.set('fp', node.fingerprint)
                if (node.network && node.network !== 'tcp') {
                    trojanParams.set('type', node.network)
                    if (node.ws) {
                        if (node.ws.path) trojanParams.set('path', node.ws.path)
                        if (node.ws.headers?.Host) trojanParams.set('host', node.ws.headers.Host)
                    } else if (node.grpc) {
                        if (node.grpc.serviceName) trojanParams.set('serviceName', node.grpc.serviceName)
                    }
                }
            ...
            case 'hysteria2':
                ...
                if (node.obfsParam) hyParams.set('obfsParam', node.obfsParam)
                if (node.obfsParam) hyParams.set('obfsParam', node.obfsParam)
                if (node.up) hyParams.set('up', String(node.up))
                if (node.down) hyParams.set('down', String(node.down))
                if (node.sni) hyParams.set('sni', node.sni)
                if (node.insecure) hyParams.set('insecure', '1')
            ...
            case 'tuic': // Fix password undefined check
                ...
                if (node.congestionControl) tuicParams.set('congestion_control', node.congestionControl)
                if (node.udpRelayMode) tuicParams.set('udp_relay_mode', node.udpRelayMode)
                if (node.sni) tuicParams.set('sni', node.sni)
                if (node.allowInsecure) tuicParams.set('allow_insecure', '1')
            ...
            case 'wireguard':
                ...
                if (node.publicKey) wgParams.set('publicKey', node.publicKey)
                if (node.ip) wgParams.set('ip', node.ip)
                if (node.mtu) wgParams.set('mtu', node.mtu.toString())
            ...
            case 'snell':
                ...
                if (node.version) snellParams.set('version', node.version.toString())
                if (node.obfs) snellParams.set('obfs', node.obfs)
...
// SingBox 格式 - 修正版
function convertToSingBox(nodes: ProxyNode[], options: ConverterOptions): string {
    const outbounds: any[] = nodes.map(node => {
        const base = {
            tag: node.name,
            server: node.server,
        ...
        switch (node.type) {
            ...
            case 'vmess':
                ...
                if (node.tls) {
                    ...
                    if (node.alpn) vmess.tls.alpn = node.alpn
                }
                if (node.network && node.network !== 'tcp') {
            ...
            case 'vless':
                ...
                if (node.tls) {
                    ...
                    if (node.alpn) vless.tls.alpn = node.alpn
                    if (node.reality) {
                        vless.tls.reality = {
                            enabled: true,
            ...
            case 'trojan':
                ...
                if (node.sni || node.tls) {
                    ...
                    if (node.alpn) trojan.tls.alpn = node.alpn
                }
                if (node.network && node.network !== 'tcp') {
...
// 辅助函数：解析插件参数
function parsePluginOpts(opts: string | Record<string, any> | undefined): any {
    if (!opts) return {}
