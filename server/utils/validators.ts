 ...
 */
export function validateNodeConfig(node: ProxyNode): ValidationResult {
    const errors: string[] = []
    ...
    // 验证必需字段
    if (!node.type) errors.push('缺少协议类型')
    if (!node.server) errors.push('缺少服务器地址')
    if (!node.port) errors.push('缺少端口号')
    ...
    switch (node.type) {
        ...
        case 'vless':
            ...
            break
        case 'ss':
            if (!node.method) errors.push('缺少加密方法')
            if (!node.password) errors.push('缺少密码')
            break
        case 'trojan':
            if (!node.password) errors.push('缺少密码')
            break
