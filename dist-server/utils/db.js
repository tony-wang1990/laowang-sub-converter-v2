    ...
    db.serialize(() => {
        ...
        db.run('ALTER TABLE subscriptions_new RENAME TO subscriptions', (err) => {
            if (err)
                ...
                console.log('✅ Subscriptions table migrated successfully');
        });
    });
}
...
export const addSubscription = (name, url, group_name = 'default', tags = '', description = '') => {
    return new Promise((resolve, reject) => {
        const stmt = db.prepare("INSERT INTO subscriptions (name, url, group_name, tags, description) VALUES (?, ?, ?, ?, ?)");
        stmt.run(name, url, group_name, tags, description, function (err) {
            if (err)
                reject(err);
            else
                resolve({ id: this.lastID, name, url, group_name, tags, description });
        });
        stmt.finalize();
    });
};
export const updateSubscription = (id, data) => {
    return new Promise((resolve, reject) => {
        ...
        const values = [data.name, data.url];
        ...
        values.push(id);
        db.run(`UPDATE subscriptions SET ${fields.join(', ')} WHERE id = ?`, values, function (err) {
            if (err)
                reject(err);
...
export const getGroups = () => {
    return new Promise((resolve, reject) => {
        db.all("SELECT DISTINCT group_name FROM subscriptions WHERE group_name IS NOT NULL ORDER BY group_name", [], (err, rows) => {
            if (err)
                reject(err);
            else
                resolve(rows.map(r => r.group_name));
        });
    });
};
export const createShortLink = (shortCode, originalUrl) => {
    return new Promise((resolve, reject) => {
        ...
        stmt.run(shortCode, originalUrl, function (err) {
            if (err)
                ...
                resolve({ id: this.lastID, short_code: shortCode, original_url: originalUrl, clicks: 0 });
        });
        stmt.finalize();
    });
};
export const getShortLinkByCode = (shortCode) => {
    return new Promise((resolve, reject) => {
...
export const deleteShortLink = (id) => {
    return new Promise((resolve, reject) => {
        db.run("DELETE FROM short_links WHERE id = ?", [id], function (err) {
            if (err)
                reject(err);
            else
                resolve({ deleted: this.changes });
...
export const incrementShortLinkClicks = (shortCode) => {
    return new Promise((resolve, reject) => {
        db.run("UPDATE short_links SET clicks = clicks + 1, updated_at = CURRENT_TIMESTAMP WHERE short_code = ?", [shortCode], function (err) {
            if (err)
                reject(err);
            else
                resolve();
